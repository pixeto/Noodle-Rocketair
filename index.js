window.loadLeverJobs = function (options) {
      ...
      for (j = 0; j < responseData[i].postings.length; j++) {
        ...
        if (groupedPostings.findIndex(findDepartment) === -1) {
          newDepartmentToAdd = {
            ...
            teams: [{ team: teamsanitizeAttribute, teamTitle: team, postings: [] }],
          }
      ...
      for (j = 0; j < groupedPostings[i].teams.length; j++) {
        ...
        for (var k = 0; k < groupedPostings[i].teams[j].postings.length; k++) {
          content +=
            ...
            '">' +
            '<a class="lever-job" href="' +
            groupedPostings[i].teams[j].postings[k].hostedUrl +
            '"><h3 class="lever-job-title">' +
            sanitizeForHTML(groupedPostings[i].teams[j].postings[k].text) +
            '</h3><h4 class="lever-team-title">' +
            sanitizeForHTML(groupedPostings[i].teams[j].teamTitle) +
            '</h4><span class="lever-job-tag">' +
            (sanitizeForHTML(groupedPostings[i].teams[j].postings[k].categories.location) || '') +
            '<img class="lever-job-icon" src="https://uploads-ssl.webflow.com/62631a10874182774143663e/64e89d80bf10fc32f311d8b7_Icon.svg"></img></span></a></li>'
        }
  ...
  if (options.includeCss) {
    ...
    addCss('https://cdn.jsdelivr.net/gh/pixeto/Noodle-Rocketair/style.css')
  }
